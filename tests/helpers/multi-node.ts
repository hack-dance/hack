...
export interface NodeRegistryStore {
  ...
  touch(opts: { readonly id: string; readonly nowIso: string }): NodeRegistrySnapshot
}
...
export function createNodeRegistryStore(opts: {
  ...
  const remove = ({ id }: { readonly id: string }) => {
    ...
    if (defaultNodeId === id) defaultNodeId = null
    return getSnapshot()
  }
...
export function deriveNodeHealth(opts: {
  ...
  if (ageMs >= opts.offlineAfterMs) return "offline"
  if (ageMs >= opts.staleAfterMs) return "stale"
  return "healthy"
}

export type NodeSelectionReason = "explicit" | "default" | "fallback" | "missing" | "stale" | "offline"
...
export function selectNode(opts: {
  ...
  const selectCandidate = (node: TestNode | null, reason: NodeSelectionReason): NodeSelection => {
    if (!node) return { node: null, reason: "missing" }
    const status = resolveStatus(node)
    if (status !== "healthy") return { node: null, reason: status }
    return { node, reason }
  }

  if (opts.desiredNodeId) {
    const node = opts.registry.nodes.find(candidate => candidate.id === opts.desiredNodeId) ?? null
    return selectCandidate(node, "explicit")
  }

  if (opts.registry.defaultNodeId) {
    const node = opts.registry.nodes.find(candidate => candidate.id === opts.registry.defaultNodeId) ?? null
    return selectCandidate(node, "default")
  }

  const statuses = opts.registry.nodes.map(node => resolveStatus(node))
  const healthyIndex = statuses.findIndex(status => status === "healthy")
  if (healthyIndex >= 0) {
    return { node: opts.registry.nodes[healthyIndex] ?? null, reason: "fallback" }
  }

  if (opts.registry.nodes.length === 0) return { node: null, reason: "missing" }
  if (statuses.every(status => status === "offline")) return { node: null, reason: "offline" }
  return { node: null, reason: "stale" }
}

export type TransportResponse<T> = { readonly ok: true; readonly data: T } | { readonly ok: false; readonly error: string }
...
export interface FakeTransport {
  ...
  request<T>(opts: { readonly nodeId: string; readonly path: string }): Promise<TransportResponse<T>>
  setNodeOffline(opts: { readonly nodeId: string }): void
  setNodeOnline(opts: { readonly nodeId: string }): void
  setResponse<T>(opts: { readonly nodeId: string; readonly response: TransportResponse<T> }): void
}
...
export function createFakeTransport(opts?: {
  ...
  const defaultResponse: TransportResponse<unknown> = opts?.defaultResponse ?? { ok: true, data: null }
