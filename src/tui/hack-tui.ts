...
export async function runHackTui({ project }: HackTuiOptions): Promise<number> {
  ...
  try {
    ...
    const logMaxLines = Math.min(
      Math.max(1, tuiLogsConfig.maxLines),
      logMaxEntries
    );
    const historyTailStep = Math.min(
      Math.max(1, tuiLogsConfig.historyTailStep),
      logMaxEntries
    );
    const projectName =
      (cfg.name ?? "").trim() ||
      defaultProjectSlugFromPath(project.projectRoot);
    const devHost = await readProjectDevHost(project);
    const projectId = await resolveProjectId({ project, projectName });
    const headerBanner = await renderHackBanner({
      trimEmpty: true,
      maxLines: 1,
    });
    ...
    const resourcesBoxMinHeight =
      resourcesLineCount + resourcesPaddingY * 2 + 2;
    ...
    const toggleServiceScope = () => {
      ...
      setToast({
        message:
          serviceScope === "all"
            ? t`${fg("#9ad7ff")("Scope")} ${dim("all services")}`
    ...
    const toggleCollapse = () => {
      ...
      setToast({
        message: collapseMultiline
          ? t`${fg("#9ece6a")("Collapsed")} ${dim("multiline")}`
    ...
    const toggleHighlight = () => {
      if (highlightEnabled) {
        ...
        if (highlightQuery.trim().length === 0) {
          setToast({
            message: t`${fg("#e0af68")("No highlight query")}`,
            durationMs: 1600,
          });
          return;
        }
    ...
    const clearSearchState = (opts: { readonly notify: boolean }) => {
      const hadQuery =
        highlightQuery.trim().length > 0 || searchQuery.trim().length > 0;
      searchMode = "live";
      searchResults = [];
      ...
      if (opts.notify && hadQuery) {
        setToast({
          message: t`${fg("#9ad7ff")("Search cleared")}`,
          durationMs: 1400,
        });
      }
    };
    ...
    const filterLocalSearchEntries = (opts: {
      ...
      const sinceTime =
        opts.since.trim().length > 0 ? parseTimeInput(opts.since) : null;
      const untilTime =
        opts.until.trim().length > 0 ? parseTimeInput(opts.until) : null;
    ...
    const renderFooter = () => {
      ...
      const copyHint = logsHasSelection
        ? t`${dim("[")}${fg("#9ad7ff")("c")}${dim("]")} copy`
        : null;
      const resultsQuery =
        searchMode === "results" && searchQuery.trim().length > 0
          ? searchQuery.trim()
          : null;
    ...
    const setActivePane = (pane: "services" | "logs") => {
      ...
      activePane = pane;
      servicesBox.borderColor =
        pane === "services" ? paneFocusBorderColor : paneBorderColor;
      logsBox.borderColor =
        pane === "logs" ? paneFocusBorderColor : paneBorderColor;
      renderHeader();
      renderMetaPanel({ runtime: currentRuntime });
      renderFooter();
      if (pane === "services") {
    ...
    const renderHeader = () => {
      ...
      const bannerLine =
        headerLabel.length > 0 ? truncateLine(headerLabel, maxLineWidth) : "";
    ...
    const renderMetaPanel = (opts: {
      ...
      const historyLabel = logStartTimestamp
        ? formatDurationShort({
            ms: Date.now() - Date.parse(logStartTimestamp),
          })
        : "n/a";
    ...
    const refreshCaddyStatus = async () => {
      ...
      const nextMappedIp = await readInternalExtraHostsIp({
    ...
    const renderResourcesPanel = (opts: { readonly targetLabel: string }) => {
      ...
      const resourcesWidth = Math.max(
        0,
        Math.floor(
          resourcesBox.width || sidebar.width || activeRenderer.width || 0
        )
      );
      ...
      const cpuLabel =
        statsSnapshot.cpuPercent !== null
          ? `${statsSnapshot.cpuPercent.toFixed(1)}%`
          : "n/a";
      const memLabel =
        statsSnapshot.memUsedBytes !== null &&
        statsSnapshot.memLimitBytes !== null
    ...
    const layoutSidebar = () => {
      ...
      let needed =
        metaHeight +
        (showResources ? resourcesBoxMinHeight : 0) +
        servicesMinHeight +
        gapCount * gapSize;
      ...
      if (sidebarHeight < needed) {
        servicesMinHeight = Math.max(
          3,
          sidebarHeight - metaHeight - gapCount * gapSize
        );
      }
    ...
    const renderServices = (runtime: RuntimeProject | null) => {
      ...
      const services = [...runtime.services.values()]
        ...
        .map((service) => {
          ...
          const runningCount = service.containers.filter(
            (c) => c.state === "running"
          ).length;
      ...
      const options = [
        { name: "All services", description: "", value: null },
        ...services,
      ];
    ...
    const renderSearchServices = (opts: {
      ...
      const selectedValue =
        searchServiceSelect.getSelectedOption()?.value ?? null;
    ...
    const refreshStats = async (opts: {
      ...
      const targetLabel =
        serviceScope === "all" || !selectedService
          ? "All services"
          : selectedService;
    ...
    const isScrollAtBottom = () => {
      const viewportHeight = Math.max(
        0,
        Math.floor(logsScroll.viewport.height || 0)
      );
      ...
      if (searchMode !== "results") {
        if (serviceScope === "selected" && selectedService) {
          activeEntries = activeEntries.filter(
            (entry) => entry.service === selectedService
          );
        }
    ...
    const mergeLogEntry = (opts: {
      readonly target: LogEntry;
      readonly entry: LogEntry;
    }) => {
      opts.target.messagePlain = `${opts.target.messagePlain}\n${opts.entry.messagePlain}`;
      const continuationChunks = buildContinuationLineChunks({
        prev: opts.target,
        entry: opts.entry,
      });
    ...
    const appendLogEntry = (entry: LogEntry) => {
      ...
      if (logState.entries.length > logState.maxEntries) {
        logState.entries.splice(
          0,
          logState.entries.length - logState.maxEntries
        );
        trimmed = true;
      }
      if (
        entry.timestamp &&
        (!logStartTimestamp || entry.timestamp < logStartTimestamp)
      ) {
    ...
    const copySelectedLogs = async () => {
      ...
      setToast({
        message: result.ok
          ? t`${fg("#9ece6a")("Copied")} ${dim("to clipboard")}`
    ...
    const fetchLogSnapshot = async (opts: {
      ...
      if (stderr && typeof stderr !== "number") {
        void consumeLogStream({
          ...
          onLine: (line) => {
            appendLogEntry(
              formatSystemLine({ message: `[history] ${line}`, tone: "warn" })
            );
          },
        });
      }
    ...
    bindSearchFieldFocus({
      field: searchServiceSelect,
      frame: searchServiceField,
    });
    ...
    const openSearchOverlay = () => {
      ...
      updateSearchRecent();
      if (selectedService) {
        const idx = searchServiceSelect.options.findIndex(
    ...
    const runSearch = async () => {
      ...
      renderMetaPanel({ runtime: currentRuntime });
      setSearchStatus({
        message: `Found ${entries.length} matches`,
        tone: "info",
      });
      searchResults = entries;
      searchSelectedIndex = 0;
    ...
    const startLogStream = async () => {
      ...
      if (logStderr && typeof logStderr !== "number") {
        void consumeLogStream({
          ...
          onLine: (line) => {
            appendLogEntry(
              formatSystemLine({ message: `[stderr] ${line}`, tone: "warn" })
            );
          },
        });
      }
    ...
    const runAction = async (opts: {
      ...
      appendLogEntry(
        formatSystemLine({
          message: `[action] ${opts.label} requested`,
          tone: "muted",
        })
      );
      const invocation = await resolveHackInvocation();
    ...
    refreshTimer = setInterval(() => void refreshRuntime(), 2000);
...
function shouldCollapseLogPrefix(opts: {
...
function buildContinuationLineChunks(opts: {
  readonly prev: LogEntry;
  readonly entry: LogEntry;
}): TextChunk[] {
  const guide = dim("│ ");
  ...
  return [guide, ...(padding ? [padding] : []), ...opts.entry.messageChunks];
}
...
function buildStyledLogText(
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    if (
      prevEntry &&
      shouldCollapseLogPrefix({ prev: prevEntry, next: entry })
    ) {
...
function collapseEntryChunks(opts: {
  readonly entry: LogEntry;
...
function truncateChunksAtNewline(opts: {
  readonly chunks: TextChunk[];
}): TextChunk[] {
  const out: TextChunk[] = [];
  for (const chunk of opts.chunks) {
    ...
    if (idx > 0) {
      out.push(
        cloneChunk({ chunk, overrides: { text: chunk.text.slice(0, idx) } })
      );
    }
    break;
  }
...
function highlightChunks(opts: {
  ...
  for (const chunk of opts.chunks) {
    ...
    while (idx !== -1) {
      if (idx > cursor) {
        out.push(
          cloneChunk({ chunk, overrides: { text: text.slice(cursor, idx) } })
        );
      }
...
function highlightLine(opts: { readonly chunks: TextChunk[] }): TextChunk[] {
  ...
  return opts.chunks.map((chunk) =>
    cloneChunk({ chunk, overrides: { bg: chunk.bg ?? bg } })
  );
}

function cloneChunk(opts: {
  readonly chunk: TextChunk;
  readonly overrides?: Partial<TextChunk>;
}): TextChunk {
  ...
  return {
    ...
    ...(opts.chunk.attributes !== undefined
      ? { attributes: opts.chunk.attributes }
      : {}),
...
const MESSAGE_TOKEN_PATTERNS: MessageTokenPattern[] = [
  {
    kind: "method",
    regex: /\b(GET|POST|PUT|PATCH|DELETE|OPTIONS|HEAD)\b/g,
    priority: 0,
  },
...
function stylePlainMessage(opts: {
  readonly message: string;
  readonly level: string;
}): TextChunk[] {
...
function colorMessageToken(opts: {
  readonly kind: MessageTokenKind;
  readonly value: string;
}): TextChunk {
  if (opts.kind === "method") {
