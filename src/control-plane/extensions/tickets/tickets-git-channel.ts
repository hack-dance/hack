...
export type TicketsGitChannel = {
  ...
  readonly repair: (input: { readonly pruneLegacyRef: boolean }) => Promise<TicketsGitRepairResult>
...
export async function createGitTicketsChannel(opts: {
  ...
  const legacyRemoteRef = refMode === "hidden" ? buildRemoteRef({ branch, refMode: "heads" }) : null
  ...
  const fetchRemoteRef = async (ref: string): Promise<{ readonly ok: true } | { readonly ok: false; readonly error: string; readonly missing: boolean }> => {
  ...
  const checkoutHead = async (input: {
    ...
    if (!localRef.ok) {
      ...
      await Bun.write(resolve(worktreeDir, ".hack/tickets/README.md"), "Tickets ref for hack-cli\n")
  ...
  const pushWithRetry = async (input: {
    ...
    const push = await runGitDir({ args: ["push", "origin", `${localBranchRef}:${remoteRef}`] })
    ...
    opts.logger.warn({ message: `git push failed, retrying after fetch: ${pushMessage}` })
    ...
    const retry = await runGitDir({ args: ["push", "origin", `${localBranchRef}:${remoteRef}`] })
    if (!retry.ok) {
  ...
  > => {
    ...
    if (!listed.ok) {
      return { ok: false, error: `git ls-files failed: ${listed.stderr.trim()}` }
    }
  ...
  const pruneWorktreeToTickets = async (): Promise<{ readonly ok: true } | { readonly ok: false; readonly error: string }> => {
    let entries: string[] = []
    try {
      entries = await readdir(worktreeDir)
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : "Failed to read tickets worktree"
      return { ok: false, error: message }
    }

    for (const entry of entries) {
      if (entry === ".hack" || entry === ".git") continue
  ...
  const inspect = async (): Promise<TicketsGitInspectResult> => {
    ...
    const hasLegacyRef = legacyRemoteRef ? await hasRemoteRef(legacyRemoteRef) : false
  ...
  const repair = async (input: { readonly pruneLegacyRef: boolean }): Promise<TicketsGitRepairResult> => {
    const checkedOut = await ensureCheckedOut()
    if (!checkedOut.ok) return checkedOut

    const repairBranch = `${branch}-repair`
    const orphan = await runGitDir({ args: ["checkout", "--orphan", repairBranch] })
    if (!orphan.ok) {
      return { ok: false, error: `git checkout --orphan failed: ${orphan.stderr.trim()}` }
    }
    ...
    const renamed = await runGitDir({ args: ["branch", "-M", repairBranch, branch] })
    if (!renamed.ok) {
      return { ok: false, error: `git branch -M failed: ${renamed.stderr.trim()}` }
    }
    ...
    if (input.pruneLegacyRef && legacyRemoteRef && checkedOut.remoteUrl) {
      const prunedLegacy = await runGitDir({ args: ["push", "origin", `:${legacyRemoteRef}`] })
      if (prunedLegacy.ok) {
        didPruneLegacy = true
      } else {
...
function normalizeBranchName(input: string): string {
  ...
  if (!trimmed) return "hack/tickets"
  return trimmed.replace(/^refs\/heads\//, "").replace(/^refs\//, "")
}

function buildRemoteRef(opts: { readonly branch: string; readonly refMode: TicketsGitRefMode }): string {
