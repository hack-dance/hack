...
export async function POST(
  ...
  const grantId = readAccountControlPlaneFormValue({
    formData,
    key: "grantId",
  });
  if (!grantId) {
    return redirectToPath(
      buildAccountControlPlanePath({
        redirectTo,
        org: redirectSelection.org,
        team: redirectSelection.team,
        project,
  ...
  const response = await fetch(
    `${config.authBrokerProxyBaseUrl}/v1/auth/projects/${encodeURIComponent(project)}/access/revoke`,
    {
      method: "POST",
