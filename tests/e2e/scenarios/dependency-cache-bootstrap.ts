...
import { downBestEffort } from "./docker-shared.ts";

/** Exercise actual mounts and installer failure before consumer replacement. */
export const dependencyCacheBootstrapScenario: Scenario = {
  name: "dependency-cache-bootstrap",
  tier: "docker",
  ...
  run: async (ctx) => {
    ...
    expectExit({
      result: image,
      codes: [0],
      message: "cached Alpine image required; no pulls",
    });
    const architecture = await runCommand({
      argv: ["docker", "info", "--format", "{{.Architecture}}"],
      cwd: ctx.tempRoot,
    });
    expectExit({
      result: architecture,
      codes: [0],
      message: "Docker architecture available",
    });
    ...
    const arch = arches[architecture.stdout.trim()];
    expect({
      that: Boolean(arch),
      message: "supported Docker architecture required",
    });
...
volumes:
  dependencies:
    labels:
      hack.e2e.cache-bootstrap-owner: ${fixture.name}
`
    );
    ...
    try {
      ...
      const volumes = await runCommand({
        argv: [
          ...
          "{{.Name}}",
          "--filter",
          `label=hack.e2e.cache-bootstrap-owner=${fixture.name}`,
        ],
        cwd: fixture.root,
      });
        ...
        if (name.startsWith(`hack-cache-${fixture.name}-dependencies-`)) {
          const inspected = await runCommand({
            argv: [
              ...
              name,
              "--format",
              '{{index .Labels "hack.e2e.cache-bootstrap-owner"}}',
            ],
            cwd: fixture.root,
          });
          expectExit({
            result: inspected,
            codes: [0],
            message: "inspect fixture volume ownership",
          });
          expect({
            that: inspected.stdout.trim() === fixture.name,
            message: "exact fixture volume ownership required",
          });
          ...
          expectExit({
            result: removed,
            codes: [0],
            message: "owned fixture volume removed",
          });
        }
      }
    }
