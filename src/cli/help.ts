...
export async function printHelpForPath(
  ...
  if (isPrettyHelpEnabled()) {
    const res = await gumFormat({
      type: "markdown",
      input: markdownWithBanner,
    });
    if (res.ok) {
...
function renderRootHelpMarkdown(cli: CliSpec): string {
  ...
  lines.push(
    "> Tip: Use `--help` after any command to see command-specific help."
  );
  lines.push("");
...
function renderEntryLines(entries: readonly RootEntry[]): string[] {
  ...
  return entries.map(
    (e) => `  ${padRight(e.invocation, width + 2)}${e.summary}`
  );
}
...
function renderOptionLines(options: readonly OptionSpec[]): string[] {
  ...
  return formatted.map(
    ({ left, right }) => `  ${padRight(left, width + 2)}${right}`
  );
}
...
function formatOption(o: OptionSpec): {
  ...
  const right = o.defaultValue
    ? `${o.description} (default: ${o.defaultValue})`
    : o.description;
  return { left, right };
}
