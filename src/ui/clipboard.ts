...
export async function copyToClipboard(opts: {
  ...
  if (!command) {
    return {
      ok: false,
      error:
        "Clipboard helper not found. Install pbcopy/wl-copy/xclip, or use the copy shortcut in your terminal.",
    };
  }
  ...
  if (!proc.stdin) {
    return {
      ok: false,
      error: `Clipboard helper (${command.label}) unavailable.`,
    };
  }
...
function resolveClipboardCommand(): ClipboardCommand | null {
  ...
  for (const candidate of CLIPBOARD_COMMANDS) {
    if (candidate.platforms && !candidate.platforms.includes(platform)) {
      continue;
    }
    if (Bun.which(candidate.command)) {
      return candidate;
    }
  }
  return null;
