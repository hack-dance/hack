...
let execMockResponder:
  | ((cmd: readonly string[]) => { exitCode: number; stdout: string; stderr: string } | null)
  | null = null
...
test("global up runs docker compose up for caddy and logging", async () => {
  ...
  expect(code).toBe(0)
  expect(runCalls.some(call => call.includes("daemon") && call.includes("start"))).toBe(true)
...
test("global status --json returns summary payload", async () => {
  ...
  const parsed = JSON.parse(output.trim()) as {
    summary: { ok: boolean; caddy_ok: boolean; logging_ok: boolean; networks_ok: boolean }
  }
