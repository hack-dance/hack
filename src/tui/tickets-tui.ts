...
import type {
  TicketEvent,
  TicketSummary,
} from "../control-plane/extensions/tickets/store.ts";
...
export async function runTicketsTui({
  ...
  if (!process.stdout.isTTY) {
    logger.error({
      message:
        "Tickets TUI requires a TTY. Run this from an interactive terminal.",
    });
    return 1;
  }
  ...
  try {
    ...
    const wrapOverlayInput = (opts: {
      readonly id: string;
      readonly child: InputRenderable;
    }) => {
    ...
    const statusSelect = new SelectRenderable(activeRenderer, {
      ...
      options: STATUS_OPTIONS.map((value) => ({
        name: STATUS_LABELS[value] ?? value,
        value,
        description: "",
      })),
    });
    ...
    const focusNextPane = (direction: number) => {
      const panes: Array<"list" | "body" | "history"> = [
        "list",
        "body",
        "history",
      ];
      ...
      const next =
        (((idx + direction) % panes.length) + panes.length) % panes.length;
    ...
    const updateTicketsList = (tickets: TicketSummary[]) => {
      if (tickets.length === 0) {
        ticketsSelect.options = [
          { name: "No tickets yet.", description: "", value: null },
        ];
    ...
    const renderMeta = (ticket: TicketSummary | null) => {
      ...
      const meta = ticket.body
        ? parseTicketBodyMeta({ body: ticket.body })
        : emptyTicketBodyMeta();
      ...
      const lines = [
        ...
        ...(ticket.projectName
          ? [t`${dim(`project: ${ticket.projectName}`)}`]
          : []),
        ...(ticket.projectId
          ? [t`${dim(`project id: ${ticket.projectId}`)}`]
          : []),
      ];
    ...
    const renderBody = async (opts: {
      ...
      if (!ticket?.body) {
        bodyText.content = ticket
          ? t`${dim("No body provided.")}`
          : t`${dim("Select a ticket.")}`;
    ...
    const refreshTickets = async () => {
      ...
      ticketsById = new Map(
        ticketsCache.map((ticket) => [ticket.ticketId, ticket])
      );
      ...
      if (mode === "new") {
        ...
        if (current) {
          void store.getTicket({ ticketId: current }).then((ticket) => {
            const idx = STATUS_OPTIONS.indexOf(ticket?.status ?? "open");
...
function renderMetaExtras(meta: TicketBodyMeta): StyledText[] {
  ...
  const acceptanceSummary = summarizeMetaItems({
    items: meta.acceptanceCriteria,
    maxItems: 3,
  });
  if (acceptanceSummary) {
    extras.push(
      t`${dim("acceptance: ")}${fg("#c0caf5")(`${acceptanceSummary}`)}`
    );
  }
...
function renderDependencyMeta(opts: {
  ...
  const extras: StyledText[] = [];
  const dependsSummary = summarizeMetaItems({
    items: opts.dependsOn,
    maxItems: 3,
  });
  if (dependsSummary) {
...
function summarizeMetaItems(opts: {
