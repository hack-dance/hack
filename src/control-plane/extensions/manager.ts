...
export class ExtensionManager {
   ...
   */
  registerExtension(opts: { readonly extension: ExtensionDefinition }): void {
    const exists = this.definitions.some(
      (def) => def.manifest.id === opts.extension.manifest.id
    );
    if (exists) {
      this.logger.warn({
   ...
   */
  listExtensions(): readonly ResolvedExtension[] {
  ...
  resolveCommand(opts: {
    ...
    const command =
      resolved.commands.find((cmd) => cmd.name === opts.commandName) ?? null;
    if (!command) {
      return null;
    }
  ...
  private resolveByExtensionId(opts: {
    ...
    for (const entry of this.resolveAll()) {
      if (entry.resolved.manifest.id === opts.extensionId) {
  ...
  private findByNamespace(opts: {
    readonly namespace: string;
  }): ResolvedExtension | null {
    for (const entry of this.resolveAll()) {
      if (entry.resolved.namespace === opts.namespace) {
        return entry.resolved;
      }
    }
    return null;
...
function resolveNamespace(opts: {
  ...
  const override =
    isRecord(extensionConfig) &&
    typeof extensionConfig.cliNamespace === "string"
      ? extensionConfig.cliNamespace
      : undefined;
  const enabled =
    !!(isRecord(extensionConfig) && extensionConfig.enabled === true) ||
    (opts.extension.manifest.id === "dance.hack.gateway" &&
      opts.config.gateway.enabled === true);
  const desired = sanitizeNamespace(override ?? preferred);

  if (
    desired.length > 0 &&
    !opts.used.has(desired) &&
    !RESERVED_NAMESPACES.has(desired)
  ) {
    return {
