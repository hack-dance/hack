...
The program needs four different operator actions, with different UX and audit implications.
...
### 3. Recovery / share repair

Use when local key material was lost, but the current project key is still trusted because another valid recovery path remains.

Behavior:

- verify the recovering actor still has access rights
- mint a fresh wrapped project-key share for the actor or replacement device
- keep bundle values and the project key unchanged
- escalate to project-key rotation if the lost device or actor is now considered untrusted

### 4. Project-key rotation
