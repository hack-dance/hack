...
async function main({ args }: { readonly args: BuildArgs }): Promise<number> {
  ...
  const version =
    typeof args.versionOverride === "string"
      ? args.versionOverride
      : pkg.version;

  if (typeof version !== "string" || version.trim().length === 0) {
    process.stderr.write(
      "Unable to determine version from package.json or --version.\n"
    );
    return 1;
  }
  ...
  await copyIfPresent({
    path: resolve(repoRoot, "assets/cut.gif"),
    destDir: gifsDir,
  });
  await copyIfPresent({
    path: resolve(repoRoot, "assets/hacker-mash.gif"),
    destDir: gifsDir,
  });
  ...
  await Bun.write(
    resolve(schemasDir, "hack.env.managed.schema.json"),
    renderProjectManagedEnvSchemaJson()
  );
  await Bun.write(
  ...
  if (gumFiles.length > 0) {
    ...
    for (const file of gumFiles) {
      await copyFile({
        from: resolve(gumSourceDir, file),
        to: resolve(gumDestDir, file),
      });
    }
  }
...
function parseArgs({
  ...
  for (const arg of argv) {
    ...
    if (arg.startsWith("--out=")) {
      ...
      if (value.length === 0) {
        return { ok: false, message: "Invalid --out (empty)" };
      }
      outDirRaw = value;
      continue;
    }
    if (arg.startsWith("--version=")) {
      ...
      if (value.length === 0) {
        return { ok: false, message: "Invalid --version (empty)" };
      }
...
async function chmodExecutable({
  path,
}: {
  readonly path: string;
}): Promise<void> {
...
async function renderChecksums({
  root,
}: {
