...
static bool publish(int directory, const char *name, const char *generation, uint64_t sequence, int health, uint64_t failures, uint64_t elapsed) {
    ...
    return ok && !renameat(directory, "status.pending", directory, name);
}
int main(int argc, char **argv) {
    ...
    if (fstatat(directory, "first-sample", &st, AT_SYMLINK_NOFOLLOW) == 0 || errno != ENOENT) return 125;
    uint64_t start = millis(CLOCK_MONOTONIC), sequence = 0, failures = 0;
    int health = 2;
    bool ever_healthy = false;
    if (!publish(directory, "status", argv[8], sequence, health, failures, 0)) return 125;
    while (!stopping) {
        ...
        ++sequence;
        // One bounded diagnostic survives later success; it never supplies current readiness.
        // On the first request only, health == 1 means success even during startup grace.
        if (sequence == 1 && !publish(directory, "first-sample", argv[8], sequence, health, failures, finished - begun)) return 125;
        if (!publish(directory, "status", argv[8], sequence, health, failures, finished - begun)) return 125;
