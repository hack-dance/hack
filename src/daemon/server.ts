...
import type {
  ShellAttachment,
  ShellMeta,
} from "../control-plane/extensions/supervisor/shell-service.ts";
...
export async function runDaemon({
  ...
  if (controlPlane.configError) {
    logger.warn({
      message: `Control plane config error: ${controlPlane.configError}`,
    });
  }
  ...
  const cache = createRuntimeCache({
    onRefresh: (snapshot) => {
      metrics.lastRefreshAtMs =
        snapshot.health.checkedAtMs ?? snapshot.updatedAtMs;
  ...
  const websocketHandlers = {
    open: (ws: ServerWebSocket<unknown>) => {
      ...
      if (state.kind === "shell") {
        ...
        ws.send(
          JSON.stringify(buildShellReadyMessage({ meta: attachment.meta }))
        );
      }
    },
    message: async (
...
async function handleRequest({
  ...
  if (url.pathname === "/v1/metrics") {
    ...
    return jsonResponse({
      ...
      started_at: new Date(metrics.startedAtMs).toISOString(),
      uptime_ms: Date.now() - metrics.startedAtMs,
      cache_updated_at: cacheUpdatedAtMs
        ? new Date(cacheUpdatedAtMs).toISOString()
        : null,
  ...
  if (url.pathname === "/v1/ps") {
    ...
    const project =
      normalizeQueryParam({ value: url.searchParams.get("project") }) ??
      composeProject;
    const branch = normalizeQueryParam({
      value: url.searchParams.get("branch"),
    });
...
function parseShellCreateInput(
  value: Record<string, unknown>
...
function parseWsMessage(opts: {
  readonly message: string | Uint8Array;
}): JobStreamMessage | null {
  const text =
    typeof opts.message === "string"
      ? opts.message
      : new TextDecoder().decode(opts.message);
  let parsed: unknown;
  try {
...
function isShellSignal(value: unknown): value is NodeJS.Signals {
  return (
    typeof value === "string" && SHELL_SIGNALS.includes(value as NodeJS.Signals)
  );
}

function buildShellReadyMessage(opts: {
...
async function flushEventsOnce(opts: {
  ...
  const next = events.filter(
    (event) => event.seq > (opts.state.eventsSeq ?? 0)
  );
...
function formatRuntimeHealth(opts: { readonly health: RuntimeHealth | null }): {
  readonly ok: boolean;
