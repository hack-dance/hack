...
function registerTools(opts: { readonly server: McpServer }): void {
  ...
  const linearCommonInput = {
    profile: z.string().describe("Linear auth profile id").optional(),
  };

  const linearIssueDirectionInput = z
    .enum(["linear", "hack"])
    .describe("Sync direction");

  const linearOwnerModeInput = z
    .enum(["hack", "linear", "both"])
    .describe("Ticket owner filter when syncing hack -> Linear")
    .optional();

  opts.server.registerTool(
    "hack.linear.setup",
    {
      title: "Setup Linear extension",
      description:
        "Enable Linear extension for a project and optionally set profile/project defaults.",
      inputSchema: {
        ...projectSelectorInput,
        ...linearCommonInput,
        projectId: z.string().describe("Default Linear project id").optional(),
        projectName: z
          .string()
          .describe("Default Linear project name")
          .optional(),
        teamId: z.string().describe("Default Linear team id").optional(),
      },
      outputSchema: toolOutputSchema,
    },
    async (input) => {
  ...
  opts.server.registerTool(
    "hack.linear.status",
    {
      title: "Linear profile status",
      description:
        "Show selected Linear profile resolution and token status (supports project override context).",
      inputSchema: {
        ...projectSelectorInput,
        ...linearCommonInput,
      },
    ...
    async (input) => {
      const hasProjectSelection =
        !!input.projectName || !!input.repoRoot || !!input.path;

      const projectCwd = hasProjectSelection
        ? await resolveProjectCwd({
            selection: toProjectSelection(input),
            cwd: process.cwd(),
            requireProjectContext: false,
          })
        : { ok: true as const, cwd: undefined };
  ...
  opts.server.registerTool(
    "hack.linear.projects.list",
    {
      title: "List Linear projects",
      description: "List projects visible to the selected Linear profile.",
      inputSchema: {
        ...projectSelectorInput,
        ...linearCommonInput,
        limit: z.number().describe("Max projects to return").optional(),
      },
      outputSchema: toolOutputSchema,
    },
    async (input) => {
      ...
      const result = await runHackCommand({
        tool: "hack.linear.projects.list",
        args,
        ...(projectCwd.cwd ? { cwd: projectCwd.cwd } : {}),
      });
  ...
  opts.server.registerTool(
    ...
    {
      title: "Bind hack project to Linear project",
      description:
        "Set or clear the default Linear profile/project mapping for a hack project.",
      inputSchema: {
        ...projectSelectorInput,
        ...linearCommonInput,
        ...
        clear: z.boolean().describe("Clear binding for the project").optional(),
      },
      outputSchema: toolOutputSchema,
    },
    async (input) => {
  ...
  opts.server.registerTool(
    ...
    {
      title: "Manual one-off issue sync",
      description:
        "Sync one issue/ticket between Linear and hack tickets in a chosen direction.",
      inputSchema: {
        ...projectSelectorInput,
        from: linearIssueDirectionInput,
        issue: z
          .string()
        ...
        syncLabels: z
          .boolean()
          .describe("Include label/tag mapping in this sync")
          .optional(),
      },
      outputSchema: toolOutputSchema,
    },
    async (input) => {
      ...
      const args = [
        ...
        ...(input.projectId ? ["--project-id", input.projectId] : []),
        ...(input.teamId ? ["--team-id", input.teamId] : []),
        ...(input.syncLabels ? ["--sync-labels"] : []),
      ];

      const result = await runHackCommand({
        tool: "hack.linear.sync.issue",
        args,
        cwd: projectCwd.cwd,
      });
  ...
  opts.server.registerTool(
    ...
    {
      title: "Manual project bulk sync",
      description:
        "Bulk sync project issues/tickets between Linear and hack (manual by default).",
      inputSchema: {
        ...projectSelectorInput,
        from: linearIssueDirectionInput,
        ...linearCommonInput,
        projectId: z.string().describe("Linear project id override").optional(),
        teamId: z.string().describe("Linear team id override").optional(),
        owner: linearOwnerModeInput,
        limit: z.number().describe("Max items to sync").optional(),
        syncLabels: z
          .boolean()
          .describe("Include label/tag mapping in this sync")
          .optional(),
...
async function resolveProjectCwd(opts: {
