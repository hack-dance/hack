...
async function readConfigObject(opts: {
  ...
  if (opts.target.scope === "global") {
    ...
    return parsed.ok
      ? { ok: true, path: opts.target.path, value: parsed.value }
      : parsed;
  }
  ...
  if (jsonText !== null) {
    ...
    return parsed.ok
      ? { ok: true, path: jsonPath, value: parsed.value }
      : parsed;
  }
...
function resolveGlobalOnlyKey(opts: {
  ...
  if (section === "extensions") {
    ...
    if (
      globalOnlyExtensions.has(
        extensionId as (typeof GLOBAL_ONLY_EXTENSION_IDS)[number]
      )
    ) {
