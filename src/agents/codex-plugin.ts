...
export async function checkHackCodexPlugin(opts: {
  ...
  const hasCodex = opts.runCodexCommand || findExecutableInPath("codex");
  return await checkNativeAgentPlugin({
    scope: opts.scope,
    pluginId: PLUGIN_ID,
    runCommand: hasCodex ? (opts.runCodexCommand ?? runCodexPluginList) : null,
    missingExecutableMessage: `Codex is not installed. After installing Codex, ${PLUGIN_GUIDANCE}`,
    inspectErrorMessage: "Could not inspect installed Codex plugins.",
    missingPluginMessage: `The Hack Codex plugin is not installed. ${PLUGIN_GUIDANCE}`,
    disabledPluginMessage:
      "The Hack Codex plugin is installed but disabled. Enable it in /plugins, then start a new session.",
    parseState: parseCodexPluginState,
  });
}
...
export async function prepareHackCodexPlugin(opts: {
  ...
  return await prepareNativeAgentPlugin({
    cleanup: async () => await removeDeprecatedHackCodexIntegration(opts),
    check: async () => await checkHackCodexPlugin(opts),
  });
}
...
function parseCodexPluginState(opts: { readonly json: string }) {
