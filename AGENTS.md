...
- Check guidance only when Hack is relevant and there is evidence of drift; use `hack agent prime --check` for a read-only inventory. Routine tasks do not require a startup audit.
...
- Content revision: `140421b48da0` (version alone is not a freshness guarantee).
...
- Read-only freshness checks are available through targeted `hack setup` checks, `hack doctor`, and `hack agent prime --check`. Plain `hack agent prime` renders current guidance without scanning integrations.
