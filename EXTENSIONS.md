...
- Enablement is per-project via `controlPlane` in `.hack/hack.config.json`.
- Built-in Gateway extension has a shortcut enablement flag: `controlPlane.gateway.enabled` implies the gateway extension.
...
`controlPlane` lives in `.hack/hack.config.json` and is parsed by
`src/control-plane/sdk/config.ts`.
...
- `hack config set 'controlPlane.extensions[\"dance.hack.gateway\"].enabled' true`
...
- Cloudflare: `hack x cloudflare tunnel-print|tunnel-setup|tunnel-start|tunnel-stop`
...
requires `controlPlane.gateway.allowWrites = true`).
...
For full usage guidance and structured workflow patterns, see `GATEWAY_API.md`.

For Cloudflare setup, see `src/control-plane/extensions/cloudflare/README.md`.
For Tailscale setup, see `src/control-plane/extensions/tailscale/README.md`.
...
Goal: co-locate manifest, commands, docs, and agent rules alongside each extension implementation.
This keeps the core CLI free of hardcoded command handling and makes it easier for third parties
to extend the system.

Current built-ins live under `src/control-plane/extensions/<name>/` and export an `extension.ts`.
Future layout (more explicit manifest + rules):

```
src/control-plane/extensions/<extension-name>/
  manifest.ts
  commands.ts
  runtime.ts
  README.md
...
`manifest.ts` would export the extension definition so `BUILTIN_EXTENSIONS` can load it without
duplicating command wiring.

## Agent rules (optional)

Extensions may ship optional agent instructions or skills (for Cursor/Claude/Codex) co-located with
the extension, so tooling can surface the right rules when a feature is enabled.
