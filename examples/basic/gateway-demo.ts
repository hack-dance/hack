...
async function main(): Promise<void> {
  ...
  if (!config.allowWrites) {
    process.stderr.write(
      "Skipping job create (set HACK_ALLOW_WRITES=1 to enable).\n"
    );
    return;
  }
...
function loadGatewayConfig(): GatewayConfig | null {
  ...
  const timeoutMs = Number.parseInt(
    process.env.HACK_JOB_TIMEOUT_MS ?? "60000",
    10
  );
...
async function printStatus(opts: {
  ...
  const res = await fetch(new URL("/v1/status", opts.baseUrl), {
