    ...
    try {
      ...
      if (!hasTicketsAgentDocSnippet({ content: existing })) {
        results.push({
          ...
          status: "error",
          path,
          message: "Missing hack tickets markers.",
        });
        continue;
      }
...
export function renderTicketsAgentDocsSnippet(): string {
  const lines = [
    ...
    "",
    "Common commands:",
    '- Create: `hack tickets create --title "..." --body-stdin [--depends-on "T-AB12CD34EF"] [--blocks "T-ZX98NM12QR"]`',
    "- List: `hack tickets list`",
    "- Tui: `hack tickets tui`",
    "- Show: `hack tickets show T-AB12CD34EF`",
    '- Update: `hack tickets update T-AB12CD34EF [--title "..."] [--body "..."] [--depends-on "..."] [--blocks "..."]`',
    "- Status: `hack tickets status T-AB12CD34EF in_progress`",
...
function resolveTicketsAgentDocPath(opts: {
  ...
  return resolve(
    opts.projectRoot,
    opts.target === "agents" ? "AGENTS.md" : "CLAUDE.md"
  );
}

function hasTicketsAgentDocSnippet(opts: {
...
function upsertSnippet(opts: {
  ...
  if (start !== -1 && end !== -1 && end > start) {
    ...
    const glued = [prefix, opts.snippet.trim(), suffix]
      .filter(Boolean)
      .join("\n\n");
